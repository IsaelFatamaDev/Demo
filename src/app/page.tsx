...
import { schemaGeneratorFlow } from "@/ai/flows/schema-generator";
...
export default function Home() {
  ...
  const handleGenerate = async (input: string) => {
    ...
    setTableData([]);
    try {
      const result = await runFlow(schemaGeneratorFlow, input);
      setJsonOutput(result.jsonSchema);
    ...
    <main className="container mx-auto p-4 md:p-8">
      <header className="mb-8 text-center">
        ...
        <p className="mt-3 text-lg text-muted-foreground">
          Ingrese una definición de base de datos para generar un esquema JSON y
          una tabla HTML.
        </p>
      </header>
